...
function convertDatabase15To32(db) {
	...
	if (fs.existsSync(path.join(cfgDir, 'banks'))) {
		for (var page in oldBankConfig) {
			if (oldBankConfig[page]) {
				for (var bank in oldBankConfig[page]) {
					if (oldBankConfig[page][bank] && oldBankConfig[page][bank].style) {
						const fullPath = path.join(cfgDir, 'banks', `${page}_${bank}.png`)
						try {
							if (fs.existsSync(fullPath)) {
								const data = fs.readFileSync(fullPath, 'base64')
								oldBankConfig[page][bank].png64 = data
							}
						} catch (e) {
...
module.exports = {
	startup: convertDatabase15To32,
	preset: convertPresetStyle,
	import: convertImport15To32,
}
