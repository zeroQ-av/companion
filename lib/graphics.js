...
graphics.prototype.drawBankImage = function (c, page, bank) {
	...
	self.system.emit('bank_upgrade_style', c)
	...
	if (c.style == 'pageup') {
		...
		if (self.page_plusminus) {
			img.drawLetter(30, 20, self.page_direction_flipped ? '-' : '+', colorWhite, 0, 1)
		} else {
			img.drawLetter(26, 20, 'arrow_up', colorWhite, 'icon')
		}
		...
		if (self.page_plusminus) {
			img.drawLetter(30, 40, self.page_direction_flipped ? '+' : '-', colorWhite, 0, 1)
		} else {
			img.drawLetter(26, 40, 'arrow_down', colorWhite, 'icon')
		}
